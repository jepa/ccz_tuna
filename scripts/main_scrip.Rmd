...
suppressMessages( # remove annoying messages from function
  ...
  proportion_hist <- bind_rows(
    lapply(h_years, data_analysis, sp = "skj", output = "NA"),
    lapply(h_years, data_analysis, sp = "yft", output = "NA"),
    lapply(h_years, data_analysis, sp = "bet", output = "NA")
  ) %>% 
    select(-h_value) %>% 
...
time_series_data <- read_csv("../results/tbl/time_series_all_esm.csv")
...
time_series_data %>%
  ...
  mutate(spp = ifelse(spp =="bet", "Bigeye tuna",
                      ifelse(spp == "skj","Skipjack tuna","Yellowfin tuna")
  ),
