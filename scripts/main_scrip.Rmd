...
# proportion_data %>% 
  ...
  mutate(per_change = round((future-present)/present*100)) %>% 
  # View()
  # mutate(per_change = ifelse(per_change>100,100,per_change)) %>% cadetblue3
  ggplot() +
  ...
  # geom_sf(data = rfmo_map %>% filter(OBJECTID == 44), aes(), fill = NA, color = "grey70") + # Just RFMO line
  geom_tile(
    aes(
      x = lon,
      y = lat,
...
# ggsave("~/Data/ccz_tuna/results/figs/relative_change_all_esm/allgrids_spp_.png",
       last_plot(),
       height = 5,
       width = 10
...
hist_connection <- time_series_data %>% 
  filter(year == 2011) %>% 
  mutate(rcp= "historical")


### Save extracted dataset
data_name_save <- paste0("../results/tbl/time_series_all_esm.csv")
...
time_series_data %>% 
  bind_rows(hist_connection) %>% 
  mutate(spp = ifelse(spp =="bet", "Bigeye tuna",
                      ifelse(spp == "skj","Skipjack tuna","Yellowtail tuna")
                      ),
         rcp = ifelse(rcp == "rcp45", "RCP 4.5",
                       ifelse(rcp == "rcp85","RCP 8.5", "Historic")
         )
         ) %>% 
  # View()
  filter(zone == "CCZ") %>% 
  ...
  labs(y = "Total biomass (10^6mt)",
       x = "Year") +
  facet_wrap(~spp, scales = "free") +
  scale_color_manual("RCP", values = c("black","blue","red")) +
  my_ggtheme_p() +
  theme(
    strip.text.x = element_text(family = "Arial", size = 14),
    axis.text.x   = element_text(family = "Arial", size = 12),
    axis.text.y   = element_text(family = "Arial", size = 12),
